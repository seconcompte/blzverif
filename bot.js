...
// URL publique du serveur (doit être exactement identique à SERVER_URL dans server.js)
const SERVER_URL =
  "https://477f5558-f863-437b-a074-fdf09960b3dc-00-v6gu9kks5091.janeway.replit.dev";
const NOTIFICATION_CHANNEL_ID = "1322369498858131507"; // Salon de notifications
const VERIFIED_ROLE_ID = "1353130129400004619"; // Rôle attribué aux comptes vérifiés
const ALT_ROLE_ID = "1353155754034466816"; // Rôle attribué aux doubles comptes (alts)

// Connexion à la base de données (pour la commande /recherche)
const db = new sqlite3.Database("./database.db", (err) => {
  if (err) console.error("Erreur DB dans bot.js:", err.message);
  else console.log("Base de données connectée dans bot.js.");
});
...
process.on("message", async (msg) => {
  ...
  if (msg.type === "verified") {
    // Cas 1 : Vérification réussie : envoie un embed et attribue le rôle vérifié
    ...
    // Cas 2/3 : Double compte détecté : envoie un embed et attribue le rôle alt
    const embed = new EmbedBuilder()
...
// --- Commande !button ---
// Réservée aux administrateurs : envoie un message public avec un bouton interactif.
// Quand un utilisateur clique sur le bouton, le bot vérifie d'abord s'il est déjà vérifié.
client.on("messageCreate", async (message) => {
  ...
  if (interaction.customId === "generate_verification_link") {
    ...
    try {
      ...
      await interaction.reply({
        content: `:warning: **Attention** : En cliquant sur ce lien, vous acceptez que votre IP soit récupérée.\n\n**Lien de vérification** : ${link}`,
        ephemeral: true,
      });
    } catch (err) {
...
// Cette commande recherche globalement les doubles comptes associés à un compte donné.
// Elle est enregistrée globalement pour être disponible sur tous les serveurs.
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "recherche") {
    ...
    // Recherche dans la DB : on cherche l'IP associée au compte, puis tous les comptes ayant cette même IP.
...
client.login("MTMxNzU1MzI0NTYxMDMxMTc5Mg.Gn_kmV.Qe99SRE9VCryBkguExs0M8kIq-DGsgkdkFDcUA"); // Remplace par le token de ton bot Discord
